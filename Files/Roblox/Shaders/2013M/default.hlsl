...
VertexOutput DefaultVS(Appdata IN)
{
 ...
 posWorld = float3(
  posWorld.x + (sin(posWorld.y/1.5)*1.5),
  posWorld.y + (sin(posWorld.z/1.5)*1.5), //- (dif.x*dif.x + dif.z*dif.z)/100.0,
  posWorld.z + (sin(posWorld.x/1.5)*1.5)
 );
    ...
    out float4 oColor0: COLOR0)
{
    ...
    float3 diffuseIntensity = saturate(ndotl) * Lamp0Color + max(-ndotl, 0) * Lamp1Color;
